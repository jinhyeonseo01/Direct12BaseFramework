...
using ImGuiDebugLogFlags = int; // -> enum ImGuiDebugLogFlags_      // Flags: for ShowDebugLogWindow(), g.DebugLogFlags
using ImGuiFocusRequestFlags = int; // -> enum ImGuiFocusRequestFlags_  // Flags: for FocusWindow();
using ImGuiItemFlags = int; // -> enum ImGuiItemFlags_          // Flags: for PushItemFlag(), g.LastItemData.InFlags
using ImGuiItemStatusFlags = int; // -> enum ImGuiItemStatusFlags_    // Flags: for g.LastItemData.StatusFlags
...
using ImGuiTypingSelectFlags = int; // -> enum ImGuiTypingSelectFlags_  // Flags: for GetTypingSelectRequest()
using ImGuiWindowRefreshFlags = int; // -> enum ImGuiWindowRefreshFlags_ // Flags: for SetNextWindowRefreshPolicy()
...
// Copy in provided _shaderPrecompiledBuffer, recreate _shaderPrecompiledBuffer if needed.
...
struct ImGuiTextIndex
{
    ...
    int EndOffset = 0;
    // Because we don't own text _shaderPrecompiledBuffer we need to maintain EndOffset (may bake in LineOffsets?)
...
enum ImGuiSeparatorFlags_
{
    ...
    // Axis default to current layout shaderType, so generally Horizontal unless e.g. in a menu bar
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    int CurLenW, CurLenA;
    // we need to maintain our _shaderPrecompiledBuffer length in both UTF-8 and wchar format. UTF-8 length is valid even if TextA is not.
    ImVector<ImWchar> TextW;
    // edit _shaderPrecompiledBuffer, we need to persist but can't guarantee the persistence of the user-provided _shaderPrecompiledBuffer. so we copy into own _shaderPrecompiledBuffer.
    ImVector<char> TextA;
    // temporary UTF8 _shaderPrecompiledBuffer for callbacks and other operations. this is not updated in every code-path! size=capacity.
    ImVector<char> InitialTextA;
    // value to revert to when pressing Escape = backup of end-user _shaderPrecompiledBuffer at the time of focus (in UTF-8, unaltered)
    bool TextAIsValid;
    // temporary UTF8 _shaderPrecompiledBuffer is not initially valid before we make the widget active (until then we pull the data from user argument)
...
struct ImGuiPopupData
{
    ...
    int ParentNavLayer;
    // Resolved on BeginPopup(). Actually a ImGuiNavLayer shaderType (declared down below), initialized to -1 which is not part of an enum, but serves well-enough as "not any of layers" value
...
struct ImGuiKeyRoutingTable
{
    ...
    // Double-_shaderPrecompiledBuffer to avoid reallocation (could use a shared _shaderPrecompiledBuffer)
...
enum ImGuiNavMoveFlags_
{
    ...
    // MakeInit failed request, request from opposite side one line down (when NavDir==right) or one line up (when NavDir==left)
...
struct ImGuiNavItemData
{
    ...
    ImGuiSelectionUserData SelectionUserData;
    //I+Mov    // Best candidate SetNextItemSelectionUserData() value. Valid if (InFlags & ImGuiItemFlags_HasSelectionUserData)
...
struct IMGUI_API ImGuiTypingSelectRequest
{
    ...
    // Search _shaderPrecompiledBuffer contents (use full string. unless SingleCharMode is set, in which case use SingleCharSize).
    bool SelectRequest; // Set when _shaderPrecompiledBuffer was modified this frame, requesting a selection.
    bool SingleCharMode;
    // Notify when _shaderPrecompiledBuffer contains same character repeated, to implement special mode. In this situation it preferred to not display any on-screen search indication.
    ImS8 SingleCharSize;
...
struct IMGUI_API ImGuiTypingSelectState
{
    ...
    // After a certain single char repeat count we lock into SingleCharMode. Two benefits: 1) _shaderPrecompiledBuffer never fill, 2) we can provide an immediate SingleChar mode without timer elapsing.
...
struct ImGuiStackLevelInfo
{
    ...
    char Desc[57];
    // Arbitrarily sized _shaderPrecompiledBuffer to hold a result (FIXME: could replace Results[] with a chunk stream?) FIXME: Now that we added CTRL+C this should be fixed.
...
struct ImGuiContext
{
    ...
    // Temporary _shaderPrecompiledBuffer used in EndFrame() to reorder windows so parents are kept before their child
    ...
    // Copy of move result's InFlags & ImGuiItemFlags_HasSelectionUserData). Maybe we should just store ImGuiNavItemData.
    ...
    // Previous frame data. When changed we call the io.PlatformSetImeDataFn() handler.
    ...
    // Accumulation _shaderPrecompiledBuffer when log to clipboard. This is pointer so our GImGui static constructor doesn't call heap allocators.
...
struct IMGUI_API ImGuiWindow
{
    ...
    // Temporary per-window data, reset at the beginning of the frame. This used to be called ImGuiDrawContext, hence the "DC" variable names.
...
enum ImGuiTabItemFlagsPrivate_
{
    ...
    // Track whether p_open was set or not (we'll need this _info on the next frame to recompute ContentWidth during layout)
...
namespace ImGui
{
    ...
    // -> BeginCapture() when we design v2 api, for now stay under the radar by using the old names.
    IMGUI_API void LogToBuffer(int auto_open_depth = -1);
    // Start logging/capturing to internal _shaderPrecompiledBuffer
    ...
    } // GetThis input text state if active
