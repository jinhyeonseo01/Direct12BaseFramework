    ...
    if (hovered)
    {
        ...
        if (!(flags & ImGuiButtonFlags_NoKeyModifiers) || (!g.IO.KeyCtrl && !g.IO.KeyShift && !g.IO.KeyAlt))
        {
            ...
            if (flags & ImGuiButtonFlags_PressedOnRelease)
            {
                ...
                {
                    const bool has_repeated_at_least_once = (item_flags & ImGuiItemFlags_ButtonRepeat) && g.IO.
                        MouseDownDurationPrev[mouse_button_released] >= g.IO.KeyRepeatDelay;
        ...
        if (g.ActiveIdSource == ImGuiInputSource_Mouse)
        {
            ...
            else
            {
                ...
                if ((release_in || release_anywhere) && !g.DragDropActive)
                {
                    ...
                    bool is_repeating_already = (item_flags & ImGuiItemFlags_ButtonRepeat) && g.IO.MouseDownDurationPrev
                        ...
                        ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    const float grab_h_pixels = ImClamp(scrollbar_size_v * ((float)size_visible_v / (float)win_size_v),
    ...
    if (held && allow_interaction && grab_h_norm < 1.0f)
    {
        ...
        const int held_dir = (clicked_v_norm < grab_v_norm)
                                 ? -1
        ...
        if (g.ActiveIdIsJustActivated)
        {
            ...
            g.ScrollbarClickDeltaToGrabCenter = (g.ScrollbarSeekMode == 0.0f)
...
void ImGui::SeparatorTextEx(ImGuiID id, const char* label, const char* label_end, float extra_w)
{
    ...
    //ImMax(padding.y, ImFloor((style.SeparatorTextSize - label_size.y) * 0.5f));
...
{
    auto data = (ImGuiGetNameFromIndexOldToNewCallbackData*)user_data;
    const char* s = NULL;
                          ...
                          const char* format, ImGuiSliderFlags flags)
{
    ...
    const bool is_already_past_limits_and_pushing_outward = is_bounded && !is_wrapped && ((*v >= v_max && adjust_delta >
                         ...
                         const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        if (g.ActiveIdSource == ImGuiInputSource_Mouse && !g.IO.MouseDown[0])
            ClearActiveID();
        else if ((g.ActiveIdSource == ImGuiInputSource_Keyboard || g.ActiveIdSource == ImGuiInputSource_Gamepad) && g.
            NavActivatePressedId == id && !g.ActiveIdIsJustActivated)
            ClearActiveID();
    }
    if (g.ActiveId != id)
        return false;
                       ...
                       const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (!temp_input_is_active)
    {
        ...
        if (make_active && temp_input_allowed)
            if ((clicked && g.IO.KeyCtrl) || double_clicked || (g.NavActivateId == id && (g.NavActivateFlags &
                            ...
                            const char* format, const void* p_clamp_min, const void* p_clamp_max)
{
    ...
    ImGuiInputTextFlags flags = ImGuiInputTextFlags_AutoSelectAll | (ImGuiInputTextFlags)
        ImGuiInputTextFlags_NoMarkEdited | (ImGuiInputTextFlags)ImGuiInputTextFlags_LocalizeDecimalPoint;
                                     ...
                                     ImGuiInputTextCallback callback, void* user_data, bool input_source_is_clipboard)
{
            ...
            ImGuiInputTextFlags)ImGuiInputTextFlags_LocalizeDecimalPoint)))
    {
        ...
        if (flags & (ImGuiInputTextFlags_CharsDecimal | ImGuiInputTextFlags_CharsScientific | (ImGuiInputTextFlags)
                                                         ...
                                                         int new_length_a)
{
    ...
    auto new_buf = (ImWchar*)(void*)g.TempBuffer.Data;
                        ...
                        ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const bool input_requested_by_nav = (g.ActiveId != id) && ((g.NavActivateId == id) && ((g.NavActivateFlags &
        ImGuiActivateFlags_PreferInput) || (g.NavInputSource == ImGuiInputSource_Keyboard)));
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        const bool is_cut = (Shortcut(ImGuiMod_Ctrl | ImGuiKey_X, f_repeat, id) ||
            Shortcut(ImGuiMod_Shift | ImGuiKey_Delete, f_repeat, id)) && !is_readonly && !is_password && (!is_multiline
            || state->HasSelection());
        const bool is_copy = (Shortcut(ImGuiMod_Ctrl | ImGuiKey_C, 0, id) ||
            Shortcut(ImGuiMod_Ctrl | ImGuiKey_Insert, 0, id)) && !is_password && (!is_multiline || state->
            HasSelection());
        const bool is_paste = (Shortcut(ImGuiMod_Ctrl | ImGuiKey_V, f_repeat, id) || Shortcut(
            ImGuiMod_Shift | ImGuiKey_Insert, f_repeat, id)) && !is_readonly;
        const bool is_undo = (Shortcut(ImGuiMod_Ctrl | ImGuiKey_Z, f_repeat, id)) && !is_readonly && is_undoable;
        const bool is_redo = (Shortcut(ImGuiMod_Ctrl | ImGuiKey_Y, f_repeat, id) || (is_osx && Shortcut(
            ImGuiMod_Ctrl | ImGuiMod_Shift | ImGuiKey_Z, f_repeat, id))) && !is_readonly && is_undoable;
        ...
        const bool is_cancel = Shortcut(ImGuiKey_Escape, f_repeat, id) || (nav_gamepad_active && Shortcut(
            ImGuiKey_NavGamepadCancel, f_repeat, id));
...
static void ColorEditRestoreH(const float* col, float* H)
{
    ...
    if (g.ColorEditSavedID != g.ColorEditCurrentID || g.ColorEditSavedColor != ImGui::ColorConvertFloat4ToU32(
...
ImGuiTypingSelectRequest* ImGui::GetTypingSelectRequest(ImGuiTypingSelectFlags flags)
{
    ...
    for (ImWchar w : g.IO.InputQueueCharacters)
    {
        ...
        if (data->SingleCharModeLock && w_len == out_request->SingleCharSize && memcmp(w_buf, data->SearchBuffer, w_len)
            == 0)
        {
    ...
    {
        auto p = (char*)(void*)ImTextFindPreviousUtf8Codepoint(data->SearchBuffer, data->SearchBuffer + buffer_len);
        *p = 0;
...
static bool IsRootOfOpenMenuSet()
{
    ...
    if (window->DC.NavLayerCurrent != upper_popup->ParentNavLayer)
        return false;
    return upper_popup->Window && (upper_popup->Window->Flags & ImGuiWindowFlags_ChildMenu) && ImGui::IsWindowChildOf(
        upper_popup->Window, window, true);
}
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    else if (want_open)
    {
        ...
        // | (want_open_nav_init ? ImGuiPopupFlags_NoReopenAlwaysNavInit : 0));
    }
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    {
        ...
        const bool has_close_button_or_unsaved_marker = (tab->Flags & ImGuiTabItemFlags_NoCloseButton) == 0 || (tab->
                      ...
                      ImGuiWindow* docked_window)
{
    ...
    if (tab_contents_visible && (tab_bar->Flags & ImGuiTabBarFlags_DrawSelectedOverline))
    {
        ...
        display_draw_list->AddLine(bb.GetTL() + ImVec2(x_offset, y_offset), bb.GetTR() + ImVec2(-x_offset, y_offset),
                                   GetColorU32(tab_bar_focused
                                                   ? ImGuiCol_TabSelectedOverline
                                                   : ImGuiCol_TabDimmedSelectedOverline), 2.0f * g.CurrentDpiScale);
    }
    RenderNavHighlight(bb, id);
