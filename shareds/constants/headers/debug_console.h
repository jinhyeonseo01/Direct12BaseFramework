...
namespace Debug
{
    ...
    public:
            ...
            while (runningAsyncWriteThread.load())
            {
                if (!waiting.load())
                {
                    ...
                    while (!(str = TextPop()).empty() && safeCount <= 10'000)
                    {
                        if (isRecord)
                            debugLogOutPut << str;
                        WriteConsoleW(GetStdHandle(STD_OUTPUT_HANDLE), (const void*)str.c_str(), (DWORD)str.size(), 0,
                                      0);
                        ++safeCount;
                    }
                ...
                else
                {
                    ...
                    if constexpr (std::is_pointer_v<T>)
                    {
                        if (value == nullptr || value == NULL)
                        {
                            ...
                            if (!runningAsyncWriteThread.load())
                            {
                                WriteConsoleW(GetStdHandle(STD_OUTPUT_HANDLE), (const void*)str.c_str(),
                                              (DWORD)str.size(), 0, 0);
                            }
                            else
                                TextPush(str);
                    ...
                    if constexpr (has_to_wstring_v<T>)
                    {
                        ...
                        if (!runningAsyncWriteThread.load())
                        {
                            WriteConsoleW(GetStdHandle(STD_OUTPUT_HANDLE), (const void*)str.c_str(), (DWORD)str.size(),
                                          0, 0);
                        }
                        else
                            TextPush(str);
        ...
        Console& operator>>(std::wstring& value)
        {
            ...
            DWORD textSize = 0;
            ReadConsoleW(GetStdHandle(STD_INPUT_HANDLE), consoleInputBuffer.data(), consoleInputBuffer.size(),
                         &textSize, nullptr);
            FlushConsoleInputBuffer(GetStdHandle(STD_INPUT_HANDLE));

            while (textSize != 0 && (consoleInputBuffer[textSize - 1] == (wchar_t)0xA || consoleInputBuffer[textSize -
                1] == (wchar_t)0xD))
                textSize--;
