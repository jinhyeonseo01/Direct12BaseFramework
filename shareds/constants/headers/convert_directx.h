...
namespace std
{
	inline std::wstring to_wstring(const DirectX::XMFLOAT4X4& matrix, UINT codePage = CP_THREAD_ACP)
	{
		...
		std::wstring str2{std::to_wstring(std::format("[[{:>6.3}, {:>6.3}, {:>6.3}, {:>6.3}],\n [{:>6.3}, {:>6.3}, {:>6.3}, {:>6.3}],\n [{:>6.3}, {:>6.3}, {:>6.3}, {:>6.3}],\n [{:>6.3}, {:>6.3}, {:>6.3}, {:>6.3}]]",
	...
	inline std::wstring to_wstring(const DirectX::SimpleMath::Rectangle& vector, UINT codePage = CP_THREAD_ACP) {
		return std::to_wstring(std::format("[pos:({}, {}), size:({}, {})](Rect)", vector.x, vector.y, vector.width, vector.height));
	}
	inline std::wstring to_wstring(const DirectX::SimpleMath::Ray& vector, UINT codePage = CP_THREAD_ACP) {
		return std::to_wstring(std::format("[pos:{}, dir:{}](Ray)", std::to_string(std::to_wstring(vector.position)), std::to_string(std::to_wstring(vector.direction))));
	}
}
/*
struct Vector2;
