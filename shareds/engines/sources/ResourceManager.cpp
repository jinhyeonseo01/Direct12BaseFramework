...
std::shared_ptr<AssimpPack> AssimpPack::Load(std::wstring path, std::wstring name, bool async, bool xFlip)
{
    auto pathString = std::to_string(path);
    this->name = name;
    this->path = path;
    this->asyncLoadThread = std::make_shared<std::jthread>(std::bind(
        [=](AssimpPack* pack, std::stop_token token, std::string path)
        {
            ...
            try
            {
                ...
                unsigned int flag = //aiProcess_MakeLeftHanded | // 왼손 좌표계로 변경
                    ...
                    // 삼각형 개선. 잘 되면 켜보기 캐시히트율을 위해 삼각형 재정렬함.
                    ...
                    //UV에 대해서 변환처리 한다고 하는거같음. 텍스쳐 이상해지면 꺼버리도록
                    aiProcess_JoinIdenticalVertices | // 중복제거 후 인덱스 버퍼 기반으로 변환
                    aiProcess_SortByPType
                    // 폴리곤을 타입별로 재정렬함. aiProcess_Triangulate 쓰면 어차피 삼각형만 남아서 필요 없음. 일단 넣어~ 
                    | aiProcess_GlobalScale;
                if (!xFlip)
                    flag |= aiProcess_MakeLeftHanded;

                pack->scene = pack->importer->ReadFile(path + "\0", flag);
                if ((pack->scene == nullptr) || pack->scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE || (pack->scene->
                    mRootNode == nullptr))
                {
