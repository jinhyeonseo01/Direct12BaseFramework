...
namespace dxe
{
    ...
    std::shared_ptr<Engine> Engine::GetEngine(int index)
    {
        ...
        if (index == -1)
        {
            Engine::_mainEngineIndex = std::clamp(Engine::_mainEngineIndex, 0,
                                                  static_cast<int>(Engine::_engineList.size() - 1));
    ...
    {
        return Engine::_mainEngineIndex = (index = std::clamp(
            index, 0, static_cast<int>(Engine::_engineList.size() - 1)));
    }
    ...
    bool Engine::DeleteEngine(std::shared_ptr<Engine> engine)
    {
        ...
        int index = std::distance(Engine::_engineList.begin(),
                                  std::find_if(Engine::_engineList.begin(), Engine::_engineList.end(),
                                               [engine](const std::shared_ptr<Engine>& eng)
                                               {
                                                   return eng == engine;
                                               }));
    ...
    {
        auto findEngine = std::find_if(Engine::_engineList.begin(), Engine::_engineList.end(),
                                       [handleName](const std::shared_ptr<Engine> engine)
                                       {
    ...
    {
        auto findEngine = std::find_if(Engine::_engineList.begin(), Engine::_engineList.end(),
                                       [hWnd](const std::shared_ptr<Engine> engine)
                                       {
    ...
    {
        this->_engineMainThread = std::make_unique<std::jthread>(
            std::bind(&Engine::ThreadExecute, this, std::placeholders::_1));
    ...
    void Engine::ThreadExecute(std::stop_token token)
    {
        ...
        try
        {
            ...
            while (!token.stop_requested()) //Frame Loop
            {
                ...
                this->deltaTime = std::min(
                    std::chrono::duration_cast<std::chrono::microseconds>(currentFrameStartTime - prevTime).count() /
                    1000000.0, deltaTimeLimitValue);
                this->deltaTimeLimit = this->deltaTime >= this->deltaTimeLimitValue;
                this->currentFrame = 1.0 / this->deltaTime;

                UpdateTitleName(std::to_wstring(std::format("{} ({}:{:0.2f})", std::to_string(_titleName), "Fps",
                                                            this->currentFrame)));

                // -- Input 동작
                InputEvent _event;
                input->DataBeginUpdate();
                while (this->_engineInputDispatcher->_inputDispatcher.try_pop(_event))
                {
                    // Input 등록
                ...
                if (this->isFrameLock)
                {
                    ...
                    if (true) // 완충된 busy waiting
                    {
                        ...
                        while (std::chrono::duration_cast<std::chrono::microseconds>(
                            std::chrono::steady_clock::now() - currentFrameStartTime).count() <= (pow(10, 6) / this->
                            targetFrame));
                    }
        ...
        {
            Debug::log << "예외 발생: " << std::format("Engine : {}, thread : {}", std::to_string(_titleName),
                                                   GetCurrentThreadId()) << "\n" << e.what() << "\n";
            Debug::log.Call();
...
#pragma endregion
            ...
            this->_hWndWNDCLASSEXW.hIcon = (HICON)LoadImageW(this->_processInstance, _iconPath.data(), IMAGE_ICON, 0, 0,
                                                             LR_LOADFROMFILE);
            this->_hWndWNDCLASSEXW.hIconSm = (HICON)LoadImageW(this->_processInstance, _iconSmallPath.data(),
...
#pragma endregion
            ...
            HWND hWnd = CreateWindowW(_handleName.data(), _titleName.data(),
                                      ...
                                      //CW_USEDEFAULT //this->_windowRect.width, this->_windowRect.height
                                      nullptr, nullptr,
