...
public:
    ...
    static std::shared_ptr<RenderTexture> Create(DXGI_FORMAT format, uint32_t width, uint32_t height,
                                                 const D3D12_HEAP_PROPERTIES& heapProperty, D3D12_HEAP_FLAGS heapFlags,
                                                 ResourceState state, Vector4 clearColor = Vector4(1, 1, 1, 1));
    static std::shared_ptr<RenderTexture> Link(ComPtr<ID3D12Resource> resource, DXGI_FORMAT format, uint32_t width,
                                               uint32_t height, ResourceState state, Vector4
                                               clearColor);
